...
- **Kubeconfig write semantics** (`internal/k8s/kubeconfig.go`): MERGE, never overwrite. grant-owned entries are named `grant-<csp>-<original>` so ownership is decidable without extra state; colliding grant entries are replaced and reported on stderr; all other entries survive (a `yaml.Node` tree is edited in place, so comments and structure are preserved). `current-context` changes only with `--set-current-context`. Atomic write via a temp file **in the same directory** + `fsync` + `rename`. Mode `0600` (an existing narrower mode is preserved; an existing group/world-readable file is re-secured with a warning), parent dir `0700`. First merge into a pre-existing file writes `<target>.grant.bak` once. Target resolution honours `$KUBECONFIG` including the list form (first non-empty entry).
- **Exec-credential cache** (`internal/k8s/execcred_cache.go`): `~/.grant/cache/execcred_<sha256(csp|fqdn|roleId|namespace)>.json`, file `0600` / dir `0700`. A cache file with looser permissions is **refused and removed**. Expiry uses `status.expirationTimestamp` **verbatim** — the SDK bakes its early-refresh buffer in once, at the point the raw DPA/STS/AKS expiry is known, and grant never re-applies it. Credentials with an absent or malformed expiry are not cacheable; an expiry more than `maxCredentialLifetime` (24h) ahead is treated as clock skew and refused.
- **R-5a (unresolved without a live tenant):** the DPA-generated kubeconfig is expected to point `users[].user.exec.command` at the official `idsec`/`ark` binary. `Kubeconfig.RewriteExecCommands` rewrites it defensively — only when the command basename matches a known official binary, carrying over only the flags grant's `exec-credential` understands (`--csp`, `--fqdn`, `--role-id`, `--organization-id`, `--namespace`); anything else passes through untouched. The real shape has not been observed.
