...
// TestNewAccessRequestServiceDisablesTransientRetry exercises the real
// constructor and asserts the policy landed on the client it publishes.
// Deleting the DisableTransientRetry call in NewAccessRequestService makes this
// test fail with 4 inbound requests — i.e. a duplicate access request in an
// approver's queue.
func TestNewAccessRequestServiceDisablesTransientRetry(t *testing.T) {
	var calls int32
	...
	resp, err := svc.httpClient.Post(t.Context(), "/api/workflows/requests", map[string]string{"a": "b"})
	if err != nil {
		t.Fatalf("Post: %v", err)
	}
